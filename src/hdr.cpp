  ...
  void MertensHDR<_TPixDepth>::merge ( const std::vector<cv::Mat> &imgs, cv::Mat &fusion ) const
{
  ...
  if( n_channels == 1 )
  {
    ...
    for ( int i = 0; i < n_images; i++ )
    {
      ...
      cv_ext::ImagePyramid img_pyr, weights_pyr;
      img_pyr.buildGaussianPyrFromImage( imgs[i], pyr_max_level + 1, cv::DataType<float>::type, true );
      weights_pyr.buildGaussianPyrFromImage( weights[i], pyr_max_level + 1, cv::DataType<float>::type, true );
  ...
  else
  {
    ...
    for ( int i = 0; i < n_images; i++ )
    {
      ...
      // For each exposure, generate a pyramid for both images and weights
      cv_ext::ImagePyramid img_pyr, weights_pyr;
      img_pyr.buildGaussianPyrFromImage( imgs[i], pyr_max_level + 1, cv::DataType<cv::Vec3f>::type, true );
      weights_pyr.buildGaussianPyrFromImage( weights[i], pyr_max_level + 1, cv::DataType<float>::type, true );
