                                                  ...
                                                  int num_directions, double lambda, bool smooth_tensor )
{
  ...
  edge_detector_->setNumEdgeDirections(num_directions);
  edge_detector_->setImage(src_img);
  dist_map_tensor_ptr = ImageTensorPtr( new ImageTensor() );
  ImageTensor &dst_map_tensor = *dist_map_tensor_ptr;
  
  MemoryAlignment align = cv_ext::MEM_ALIGN_NONE;

#if defined(D2CO_USE_AVX)
  align = cv_ext::MEM_ALIGN_32;
#elif defined(D2CO_USE_SSE)
  align = cv_ext::MEM_ALIGN_16;
#endif
  
  dst_map_tensor.create(src_img.rows, src_img.cols, num_directions, DataType<float>::type, align );
...
//   cv_ext::BasicTimer timer;
  ...
  for( int i = 0; i < num_directions; i++ )
  {
    ...
    cv::distanceTransform ( edge_map, dst_map_tensor[i], dist_type_, mask_size_ );
    if( dist_thresh_ > 0 )
      dst_map_tensor[i].setTo(dist_thresh_, dst_map_tensor[i] > dist_thresh_ );
...
//   timer.reset();
  ...
  #pragma omp parallel if( parallelism_enabled_ )
  {
    ...
    for( int i = 0, prev_i = num_directions - 1; i < num_directions; i++, prev_i++ )
    {
      ...
      Mat prev_map = dst_map_tensor[prev_i].rowRange(row_ranges[i_th].first, row_ranges[i_th].second),
          cur_map = dst_map_tensor[i].rowRange(row_ranges[i_th].first, row_ranges[i_th].second);
    ...
    for( int i = num_directions - 1, next_i = 0; i >= 0; i--, next_i-- )
    {
      ...
      Mat cur_map = dst_map_tensor[i].rowRange(row_ranges[i_th].first, row_ranges[i_th].second),
          next_map = dst_map_tensor[next_i].rowRange(row_ranges[i_th].first, row_ranges[i_th].second);
...
//   counter++;
  ...
  if( smooth_tensor )
    smoothTensor(dist_map_tensor_ptr);
}
                                                  ...
                                                  int num_directions, double lambda, bool smooth_tensor )
{
  ...
  dist_map_tensor_ptr = ImageTensorPtr( new ImageTensor() );
  edgels_map_tensor_ptr = ImageTensorPtr( new ImageTensor() );
  ImageTensor &dst_map_tensor = *dist_map_tensor_ptr,
              &edgels_map_tensor = *edgels_map_tensor_ptr;

  dst_map_tensor.create(src_img.rows, src_img.cols, num_directions, DataType<float>::type );
  ...
  for( int i = 0; i < num_directions; i++ )
  {
    Mat edge_map;
    edge_detector_->getDirectionalEdgeMap(i,edge_map);
    distanceMapClosestEdgels( edge_map, dst_map_tensor[i], edgels_map_tensor[i] );
  }
...
void DistanceTransform::computeDistanceMapTensor( const Mat &src_img,
                                                  ImageTensorPtr &dist_map_tensor_ptr,
                                                  ImageTensorPtr &x_dist_map_tensor_ptr,
                                                  ImageTensorPtr &y_dist_map_tensor_ptr,
                                                  ImageTensorPtr &edgels_map_tensor_ptr,
                                                  int num_directions, double lambda, bool smooth_tensor )
{
  DistanceTransform::computeDistanceMapTensor( src_img, dist_map_tensor_ptr, edgels_map_tensor_ptr,
                                               num_directions, lambda, smooth_tensor );

  x_dist_map_tensor_ptr = ImageTensorPtr( new ImageTensor() );
  y_dist_map_tensor_ptr = ImageTensorPtr( new ImageTensor() );

  ImageTensor &edgels_map_tensor = *edgels_map_tensor_ptr,
              &x_dist_map_tensor = *x_dist_map_tensor_ptr,
              &y_dist_map_tensor = *y_dist_map_tensor_ptr;
...
//WARNING Commented out parallelization
//   #pragma omp parallel for schedule(static) if( parallelism_enabled_ )
  for( int i = 0 ; i < num_directions; i++ )
  {
    for( int y = 0; y < h; y++)
...
//     cv_ext::showImage(y_dist_map_tensor[i], "y_dst_map_tensor[i]");
  ...
  if( smooth_tensor )
  {
    smoothTensor(x_dist_map_tensor_ptr);
    smoothTensor(y_dist_map_tensor_ptr);
  }
}
...
void DistanceTransform::distanceMapClosestEdgels(const Mat& edge_map, Mat& dist_map, Mat& closest_edgels_map)
{
  ...
  // The labels_img  == VORONOI DIAGRAM == matrix groups all the pixel nearest to a edgels with a common label (ie., an ID), 
  // shared with the edgels itself  
  cv::distanceTransform(edge_map, dist_map, labels_img, dist_type_, mask_size_, DIST_LABEL_PIXEL );
  // apply threshold == Sets all or some of the array elements to the specified value.
  if( dist_thresh_ > 0 )
    dist_map.setTo(dist_thresh_, dist_map > dist_thresh_ );
//total == The method returns the number of array elements (a number of pixels if the array represents an image).
  int labels_size = edge_map.total() - countNonZero(edge_map) + 1;
  // label_size == zero values == empty space == not edgels
  ...
  for( int y = 0; y < labels_img.rows; y++)
  {
    ...
    {
        // if elem of row == 0 ==> empty space ==> !edgel
      if( !(*em_row_p) )
          //*label_row_p = label_row_p[1 2 3 4 99] 
          // add point2f to not edgel element
        // choose one closest not edgel from every label group fuck it
  ...
  for( int y = 0; y < labels_img.rows; y++)
  {
    // row of closest_edgels_map
    Point2f *points_row_p = closest_edgels_map.ptr<Point2f>(y);
    // labels row == constant!!!!! == doesn't becomes a zero with new loop.
    const u_int32_t *label_row_p = labels_img.ptr<u_int32_t>(y);
    for( int x = 0; x < labels_img.cols; x++, label_row_p++, points_row_p++ )
        //assign point that has minimum distance to edge
        // edgel contains points num of element represents label group
        // so *label_row_p gets label group and takes Point coordinates of closest point from
        //edgels == edgels represents the closest points to the edgel point from some label group
...
//       {
//         Mat dbg_img = Mat(edge_map.size(), CV_8UC3);
...
void DistanceTransform::smoothTensor( ImageTensorPtr &dist_map_tensor_ptr )
{
  int num_directions = dist_map_tensor_ptr->depth();
  ImageTensorPtr smoothed_tensor_ptr = ImageTensorPtr( new ImageTensor() );
  smoothed_tensor_ptr->create(dist_map_tensor_ptr->rows(), dist_map_tensor_ptr->rows(), 
  ...
  for( int i = 0; i < num_directions; i++ )
  {
    ...
    (*smoothed_tensor_ptr)[i] =  0.25*dist_map_tensor_ptr->at(prev_i) +
                                 0.5*dist_map_tensor_ptr->at(i) +
                                 0.25*dist_map_tensor_ptr->at(next_i);
  }
  dist_map_tensor_ptr = smoothed_tensor_ptr;
}
