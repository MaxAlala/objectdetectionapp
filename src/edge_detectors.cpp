...
void LSDEdgeDetector::getDirectionalEdgeMap( int i_dir, cv::Mat &edge_map )
{
  ...
  if( num_directions_ > 1 )
  {
    //create empty map
    edge_map = Mat( map_size_, DataType<uchar>::type, (white_bg_?Scalar(255):Scalar(0)));
    //draw segments of a certain direction
...
{
  cv_ext::ImagePyramid img_pyr;
  img_pyr.buildGaussianPyrFromImage( src_img, pyr_num_levels_ );
                                                     ...
                                                     vector<float> &normals )
{
  ...
  for(int i = 0; i < seg_size; i++)
  {
    ...
    //if sin != 0 
    if( sin_ang )
        // normal cause dot product == 0 with dx dy
