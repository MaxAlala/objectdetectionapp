...
void StereoRectification::setCameraParameters ( const PinholeCameraModel cam_models[2], 
                                                const Mat& r_mat, const Mat& t_vec )
{
  if ( cam_models[0].imgSize() != cam_models[1].imgSize() )
    throw std::invalid_argument("The two cameras models should have the same image size");
  ...
  if( r_mat.rows != 3 || r_mat.cols != 3 || t_vec.rows != 3 || t_vec.cols != 1 )
    throw std::invalid_argument("Stereo extrinsics parameters should be provided by means of a 3x3 rotation matrix and a 3X1 translation vector");
...
void StereoRectification::getRegionsOfInterest ( Rect roi[2] )
{
  for ( int k = 0; k < 2; k++ )
    roi[k] = rect_roi_[k];
}

void StereoRectification::rectifyImagePair ( const Mat imgs[2], Mat rect_imgs[2] )
{
...
void StereoRectification::getCamModels( PinholeCameraModel cam_models[2] )
{
...
void StereoRectification::getCamDisplacement ( Point2f &cam_shift )
{
  cam_shift = Point2f(0,0);
  if ( !proj_mat_[0].empty() && !proj_mat_[1].empty() )
  {
