...
//           cout<<"("<<r<<","<<c<<") : |" <<d<<" - "<<z<<" | = "<<diff<<endl;
    ...
    switch(key)
    {
      ...
      case 'r':
        ...
        break;
      case 'g':
        light_pos.x = rand() - RAND_MAX/2;
        light_pos.y = rand() - RAND_MAX/2;
        light_pos.z = rand() - RAND_MAX/2;
        
        if(light_pos.x || light_pos.y || light_pos.z )
          light_pos /= cv_ext::norm3D(light_pos);
        else
          light_pos = cv::Point3f(1,0,0);
        obj_model.setLightPos(light_pos);
        break;
